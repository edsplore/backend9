...
from api.schemas.responses import FetchManagerDashboardTrainingPlansResponse, FetchManagerDashboardModulesResponse, FetchManagerDashboardSimultaionResponse, ManagerDashboardAggregateDetails , ManagerDashboardTrainingEntityTableResponse
...
class ManagerController:
    ...
    async def get_manager_dashboard_data(self, request: FetchManagerDashboardTrainingPlansRequest) -> ManagerDashboardAggregateDetails:
        ...
        try:
            data = await self.service.get_manager_dashboard_data(request.user_id, request.reportee_user_ids)
    ...
    async def fetch_manager_dashboard_table_data(self, request: FetchManagerDashboardTrainingEntityRequest) -> ManagerDashboardTrainingEntityTableResponse:
        ...
        # reporting_userIds= ['trainee1', 'trainee2']
        logger.info(f"Fetching manager dashboard simulations for user_id={request.user_id}, reporting_userIds={request.reportee_user_ids}")
        try:
            response = await self.service.fetch_manager_dashboard_training_entity_data(request.user_id, request.reportee_user_ids, request.type, request.pagination)
            logger.debug(f"Manager dashboard simulations: {response}")
            return response
        except Exception as e:
            logger.error(f"Error fetching manager dashboard simulations: {str(e)}", exc_info=True)
