...
from api.schemas.responses import (FetchAssignedPlansResponse, Stats,
                                   StatsData,
                                   AdminDashboardUserActivityStatsResponse,
                                   CreateUserResponse)
...
class UserService:
    ...
    async def get_admin_dashboard_user_stats(
            self,
            user_id: str) -> List[AdminDashboardUserActivityStatsResponse]:
        ...
        mock_user_list = [{
            "user_id":
            "67f6286b75b9226a9afabfaa",
            "first_name":
            "Anmol",
            "last_name":
            "Rishi",
            "email":
            "anmol_test@yopmail.com",
            "phone_no":
            "+919654115965",
            "division":
            "EverAI Labs",
        ...
        try:
            date_util = DateUtils()
            admin_dashboard_user_stats_list: List[
                AdminDashboardUserActivityStatsResponse] = []
            for user in mock_user_list:
                if user.get("user_id"):
                    ...
                    user_object["name"] = user.get(
                        "first_name") + " " + user.get("last_name")
                    user_object["email"] = user.get("email")
                    user_object[
                        "role"] = "TEST_ROLE"  ## need to devise a method how to identify teh exact role name from list of roles mentioned for user
                    user_object["division"] = user.get("division")
                    user_object["department"] = user.get("department")
                    user_object[
                        "addedOn"] = date_util.convert_to_human_readable(
                            user.get("created").get("time"))
                    ...
                    if assignments_with_stats:
                        ...
                        if users_assignments_stats and users_assignments_stats.simulation_completed:
                            ...
                            user_object[
                                "assignedSimulations"] = total_simulations
                            user_object["completionRate"] = math.ceil(
                                users_assignments_stats.simulation_completed.
                                percentage)
                        if users_assignments_stats and users_assignments_stats.timely_completion:
                            user_object["adherenceRate"] = math.ceil(
                                users_assignments_stats.timely_completion.
                                percentage)
                        user_object["averageScore"] = math.ceil(
                            users_assignments_stats.average_sim_score)

                    admin_dashboard_user_stats_list.append(
                        AdminDashboardUserActivityStatsResponse(**user_object))
        ...
        try:
            ...
            if existing_user:
                ...
                if last_logged_in_at:
                    ...
                    logger.warning(
                        f"lastLoggedInAt not found for user {user_id}.")

                # checking if last loggedIn time is more than 24 hours
